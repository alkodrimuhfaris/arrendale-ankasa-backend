...
module.exports = {
    getDetailFlightByFlightId: async (req, res) => {
        ...
        try {
            ...
            if(data.length > 0) {
                return responseStandard(res, `detail Flight by flight id: ${flight_id}`, {data})
            } else {
                return responseStandard(res, 'Class flight Not found', {}, 401, false)
            }
        } catch (e) {
    ...
    getAllDetailFlight: async (req, res) => {
        try{
            ...
            const data = await detailFlightModel.getFlightDetail(limiter)
    ...
    createFlightDetail: async (req, res) => {
        try {
            ...
            if (data.insertId) {
                results = {id:data.insertId, ...results}
    ...
    deleteFlightDetail: async (req, res) => {
        try {
            ...
            const data = await detailFlightModel.deleteFlightDetail({id})
            if(data.affectedRows){
                return responseStandard(res, `Flight Detail Has been deleted`, {})
            } else {
                return responseStandard(res, 'Flight Detail Not found', {}, 401, false)
            }
        } catch (e) {
            return responseStandard(res, e.message, {}, 401, false)
        }
