...
const facilityModel = require('../../models/admin/facilities')
...
module.exports = {
            ...
            try {
                if (err instanceof multer.MulterError) {
                  ...
                  if (error) {
                      ...
                      let { name } = results
                    //   const check = await facilityModel.countFacility(name)
    ...
    getFacility: async (req, res) => {
        try {
            const data = await facilityModel.getFacility()
            if (data.length) {
    ...
    getFacilityById: async (req, res) => {
        let { id } = req.params
        try {
            const data = await facilityModel.getFacilityByConditions({ id })
            if(data.length > 0) {
            ...
            try {
                if (err instanceof multer.MulterError) {
                  ...
                  if (error) {
                      ...
                      let data = await facilityModel.updateFacility(results, id)
    ...
    deleteFacility: async (req, res) => {
        ...
        // console.log(uid)
        try {
            const data = await facilityModel.deleteFacility({id: flight_id})
            if(data.affectedRows){
