...
module.exports = {
    getProfile: async (req, res) => {
        let { id } = req.user
        try {
            const data = await adminModel.getDetailProfile({ id })
            if(data.length > 0) {
    ...
    updateProfile: async (req, res) => {
        ...
        const schema = joi.object({
            ...
            reset_code: joi.string(),   //jangan diisi, ini hanya untuk reset password
        })
        ...
        let { username, email, password, phone_number, city, address, postal_code } = results
            try {
                if (password) {
                    ...
                    const update = await adminModel.updateUser(results, id )
        ...
        uploadHelper(req, res, async function(err) {
            try {
                if (err instanceof multer.MulterError) {
                  ...
                  let data = await adminModel.updateUser(results, uid)
    ...
    deleteProfile: async (req, res) => {
        ...
        // console.log(uid)
        try {
            const data = await adminModel.deleteUser({id: uid})
            if(data.affectedRows){
